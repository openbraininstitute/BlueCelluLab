    ...
    for sec in secs:
        ...
        for x in xs:
            ...
            for mech, vardict in dens.items():
                seg_mobj = getattr(seg, mech, None)

                ref_vars = set()
                if seg_mobj is not None:
                    ref_vars = {a[5:] for a in dir(seg_mobj) if a.startswith("_ref_")}

                if not ref_vars:
                    ref_vars = set(vardict.keys())

                if ref_vars:
                    mech_map[mech] = sorted(ref_vars)
            pp_map = {}
